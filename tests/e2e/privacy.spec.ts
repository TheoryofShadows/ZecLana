...
// Use the "Solana → ZEC" preset so the destination chain is Zcash and the
// recipient address drives the privacy assessment.
test.describe("privacy meter", () => {
  ...
  test("transparent recipient flags a privacy leak with a Zashi handoff", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /Shield your ZEC in Zashi/i })).toBeVisible()
  })
